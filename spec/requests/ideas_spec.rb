describe 'アイデア登録API' do
  context 'カテゴリ名が存在しないアイデアを登録しようとしたとき' do
    it 'カテゴリとアイデアが登録される' do
      ...
      expect { post '/api/ideas', params: valid_params }.to change { Category.count }.by(+1).and change {
                                                                                                   Idea.count
                                                                                                 }.by(+1)
  ...
  context 'カテゴリ名が存在するアイデアを登録しようとしたとき' do
    it '既存のカテゴリ名にアイデアが登録される' do
      ...
      expect { post '/api/ideas', params: valid_params }.to change { Category.count }.by(0).and change {
                                                                                                  Idea.count
                                                                                                }.by(+1)
      expect(response.status).to eq(201)
    end
  ...
  context 'カテゴリ名が空のアイデアを登録しようとしたとき' do
    it '登録できない' do
      ...
      expect { post '/api/ideas', params: invalid_params }.to change { Category.count }.by(0).and change {
                                                                                                    Idea.count
                                                                                                  }.by(0)
